...
(async()=>{
 ...
 try{
  ...
  for(const count of [1,10,55]){
   ...
   assert.ok(Math.abs(dimensions.width-50*96/25.4)<1);assert.ok(dimensions.scroll<=Math.ceil(dimensions.width));assert.ok(dimensions.height>previousHeight);previousHeight=dimensions.height;
   assert.equal(await page.locator('#app').isVisible(),false);
   const pdf=await page.pdf({preferCSSPageSize:true,displayHeaderFooter:false,...(count===10?{path:path.join(os.tmpdir(),'chd-bill-58mm.pdf')}:{})});
   const text=pdf.toString('latin1'),box=text.match(/\/MediaBox\s*\[\s*0\s+0\s+([\d.]+)\s+([\d.]+)/);
   assert.ok(box,'PDF page dimensions present');assert.ok(Math.abs(Number(box[1])-58*72/25.4)<1,'58mm paper width');
   assert.equal((text.match(/\/Type\s*\/Page\b/g)||[]).length,1,'one roll page');
   if(count===10){assert.equal(await page.locator('.receipt-total').innerText(),'Tổng tiền:\n342.000 đ');await page.locator('#print').screenshot({path:path.join(os.tmpdir(),'chd-bill-58mm.png')});}
  }
  console.log('PASS: 58mm PDF page, 50mm content, 1/10/55 items, dynamic height, total, no app UI or horizontal overflow.');console.log('Preview: '+path.join(os.tmpdir(),'chd-bill-58mm.png'));
