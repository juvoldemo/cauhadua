...
document.addEventListener('click',async e=>{const b=e.target.closest('button');if(!b)return;const d=b.dataset;try{
 ...
 if(b.id==='connectusb'){b.disabled=true;try{await connectPrinter();toast('Đã chọn máy in USB. Bấm In bill để in.');render();}finally{b.disabled=false;}}
 if(b.id==='systemprinter'){useSystemPrinter();render();}
