...
let staff=null,expiresAt=0,expiryTimer;
...
async function api(url,options){const r=await fetch(url,options);if(!r.ok){if(r.status===401)lockStaff();let e;try{e=await r.json()}catch{}throw new Error(e?.error||'Không thể xử lý. Vui lòng thử lại.')}return r.json()}
...
document.addEventListener('click',async e=>{const b=e.target.closest('button');if(!b)return;const d=b.dataset;try{
 ...
 if(d.quantityOrder){
   ...
   try{
     ...
     try{orders=await api('/api/orders');}catch{}
     throw error;
 ...
 if(d.removeOrder){const order=orders.find(o=>o.id===d.removeOrder),index=Number(d.removeIndex),item=order?.items[index];if(!item||order.paid)return;if(!confirm(`Xóa ${item.qty} × ${item.name} khỏi hóa đơn bàn ${order.table}?`))return;b.disabled=true;try{await api('/api/orders/'+encodeURIComponent(order.id)+'/items/'+index,{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({expectedItems:order.items})});toast('Đã xóa món và cập nhật tổng tiền ✓');}finally{orders=await api('/api/orders');render();}}
 if(b.id==='deletebill'){
...
async function syncOrders(){
 if(!staff||syncing||sending||document.hidden)return;
 syncing=true;
 try{
   const [next,catalog]=await Promise.all([api('/api/orders'),api('/api/catalog')]);
   if(!staff)return;
...
async function init(){
 render();
 try{acceptSession(await api('/api/staff/session'));}catch(error){if(staff)toast(error.message);}
 await syncOrders();
 // Short requests work across serverless instances; no process-local SSE channels.
 setInterval(syncOrders,4000);
 document.addEventListener('visibilitychange',()=>{if(!document.hidden){if(staff&&Date.now()>=expiresAt)lockStaff();else syncOrders();}});
}init();

function renderLogin(message=''){ $('#app').innerHTML=`<main class="staff-login"><div class="brand-mark">♧</div><h1>Cầu Hà Dừa</h1><h2>Đăng nhập nhân viên</h2><p>Nhập mã nhân viên để gọi món. Phiên đăng nhập có hiệu lực 8 tiếng.</p><form id="staff-login"><label for="staff-code">Mã số nhân viên</label><input id="staff-code" name="code" type="password" inputmode="numeric" pattern="[0-9]+" autocomplete="current-password" required autofocus placeholder="Nhập mã nhân viên"><p id="staff-error" role="alert">${esc(message)}</p><button class="primary">Đăng nhập →</button></form><a href="/admin">Đăng nhập quản trị</a></main>`;}
function lockStaff(){staff=null;expiresAt=0;clearTimeout(expiryTimer);orders=[];connected=false;close();$('#print').innerHTML='';renderLogin('Vui lòng nhập mã nhân viên để tiếp tục.');}
function acceptSession(data){staff=data.employee;expiresAt=data.expiresAt;clearTimeout(expiryTimer);expiryTimer=setTimeout(lockStaff,Math.max(0,expiresAt-Date.now()));render();}
document.addEventListener('submit',async e=>{if(e.target.id!=='staff-login')return;e.preventDefault();const form=e.target,button=form.querySelector('button');button.disabled=true;try{const data=await api('/api/staff/session',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code:new FormData(form).get('code')})});form.reset();acceptSession(data);await syncOrders();}catch(error){if($('#staff-error'))$('#staff-error').textContent=error.message;}finally{button.disabled=false;}});
document.addEventListener('click',async e=>{if(!e.target.closest('#staff-logout'))return;try{await api('/api/staff/session',{method:'DELETE'});lockStaff();}catch(error){toast(error.message);}});
